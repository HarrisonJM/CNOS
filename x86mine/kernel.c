...
static const size_t VGA_WIDTH = 80; //coloumns
...
void terminal_scroll()
{
	...
	for(i = 0; i < VGA_HEIGHT; ++i) //rows height
	{
		for(j = 0; j < VGA_WIDTH; ++j) //coloumns, width
		{
			const size_t index = (i+1) * VGA_WIDTH + j;
			const size_t index2 = i * VGA_WIDTH + j; //old buffer
			terminal_buffer[index2] = terminal_buffer[index];
		}
	}
